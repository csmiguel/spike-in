...
ktu_spike_its <- read.csv("output/spikein_summary_its.csv")$OTU

# identify internal controls
int_controls <- "mcs168"

#open connection
sink(file = "output/filter_phyloseq_ITS.txt")
# start filtering
psITS_ktu_filt <-
  ps_itsktu %>%
  ...
  {phyloseq(
    ...
    tax_table = merge_taxtable(ps_itsktu_spike, ., ps_itsktu),
    sample_data(ps_itsktu),
...
saveRDS(psITS_ktu_filt, "data/intermediate/psITS_ktu_filt.rds")
saveRDS(psITS_ktu_filt_rel, "data/intermediate/psITS_ktu_filt_rel.rds")
