...
obs <- readRDS("data/intermediate/abs_abundances.rds") %>%
  drop_na() %>%
  select(sample, code, wild_reads_16s, wild_reads_its, imte_reads,
    allo_reads, yarrowia_reads) %>%
  pivot_longer(cols = c(wild_reads_16s, wild_reads_its, imte_reads,
    allo_reads, yarrowia_reads),
...
my_cols <- c("#2ca25f", "#b2e2e2", "#66c2a4" , "#fee8c8", "#fdbb84")
p1 <-
  ggplot() +
  geom_col(data = obs_bact,
    aes(x = sample, y = -reads, fill = nature), position="stack") +
  geom_col(data = obs_fung,
    aes(x = sample, y = reads, fill = nature), position="stack") +
